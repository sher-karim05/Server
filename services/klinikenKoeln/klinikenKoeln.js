...
let klinikenKoeln = async (cluster,page, positions,levels) => {
  try {
    ...
    while (nextPage) {
      ...
      let jobLinks = await page.evaluate(() => {
        return Array.from(
          document.querySelectorAll("div.job_description > a")
        ).map((el) => el.href);
      });
      ...
      let bottomNextLink = await page.evaluate(() => {
        return document.querySelector("a.page-link.next");
      });
      if (bottomNextLink) {
        await page.click("a.page-link.next");
        nextPage = true;
      } else {
        nextPage = false;
      }
      ...
      cluster.queue(async ({ page }) => {
        ...
        let link = await page.evaluate(() => {
          let lnk = document.querySelector(
            "#btnbox > div > div > a.btnInfoBoxAction.jcdBtnApplication.btn.btn-jcd-green.btn-apply-now.btn-sm"
          );
