...
let hagenn = async (cluster,page, positions,levels) => {
  try {
      ...
      cluster.queue(async ({ page }) => {
        ...
        let jobLinks = await page.evaluate(() => {
          return Array.from(
            document.querySelectorAll(
              "#c28110 > div > section > div > div > div > div > article > a"
            )
          ).map((el) => el.href);
        });
        ...
        let bottomNextLink = await page.evaluate(() => {
          return document.querySelector(
            "#c28110 > div > div > nav > ul > li.pagination__item.pagination__item--jump.pagination__item--next > a"
          );
        });
        if (bottomNextLink) {
          await page.click(
