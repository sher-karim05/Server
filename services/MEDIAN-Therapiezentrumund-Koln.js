...
let MEDIAN_Therapiezentrumund_Koln = async (cluster,page,positions,levels) => {
  try {

    await page.goto("https://www.median-kliniken.de/de/karriere/stellenangebote-bewerbung/stellenangebote.html?filter%5Bcountr%5D=K%C3%B6ln", {
      waitUntil: "load",
      timeout: 0,
    });
    ...
    for (let jobLink of jobLinks) {
      cluster.queue(async ({ page }) => {
        let job = {
          title: "",
          location: "",
          hospital: "MEDIAN Therapiezentrum und Adaptionshaus Köln",
          link: "",
          level: "",
          position: "",
          city: "Köln",
        ...
        if (positions.map(el => el.toLwerCase()).includes(job.position.toLowerCase())) {
          await save(job);
        }
      });
    }
...
export default MEDIAN_Therapiezentrumund_Koln;