...
let MEDIANTherapiezentrum_Willich = async (cluster,page,positions,levels) => {
  try {

    await page.goto("https://www.median-kliniken.de/de/karriere/stellenangebote-bewerbung/stellenangebote.html?filter%5Bcountr%5D=Willich", {
      waitUntil: "load",
      timeout: 0,
    });
    ...
    for (let jobLink of jobLinks) {
      cluster.queue(async ({ page }) => {
        let job = {
          title: "",
          location: "",
          hospital: "MEDIAN Therapiezentrum Haus Willich",
          link: "",
          level: "",
          position: "",
          city: "Willich",
        ...
        if (positions.map(el.toLowerCase()).includes(job.position.toLowerCase())) {
          await save(job);
        }
      });
    }
...
export default MEDIANTherapiezentrum_Willich;