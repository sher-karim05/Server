...
let lengerich = async (cluster,page, positions,levels) => {
  try {
    ...
    let pages = await page.evaluate(() => {
      return [...document.querySelectorAll(".pagination__list > li  > a ")].map(el => el.href).slice(1,3);
    });
    let jobLinks = [];
    for (let pg of pages) {
      ...
      cluster.queue(async ({ page }) => {
        ...
        let email = await page.evaluate(() => {
          let eml = document.querySelector(
            "#c101670 > div > section.content-block-list > div > article:nth-child(5) > div > div > div "
          );
          return eml
