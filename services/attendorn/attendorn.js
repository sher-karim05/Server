...
let wipperfuerth = async (cluster,page, positions, levels) => {
  try {

    await page.goto(
      "https://www.helios-gesundheit.de/kliniken/attendorn/unser-haus/karriere/stellenangebote/",
      {
        waitUntil: "load",
        timeout: 0,
      }
    ...
    let jobLinks = await page.evaluate(() => {
      ...
      // variable to hold collection of all book titles and price
      ...
      cluster.queue(async ({ page }) => {
        ...
        let email = await page.evaluate(() => {
          let eml = document.querySelector(
            "#c74376 > div > section.content-block-list > div > article:nth-child(5) > div > div"
          );
          return eml
