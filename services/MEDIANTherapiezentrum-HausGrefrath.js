...
let MEDIANTherapiezentrum_HausGrefrath = async (cluster,page,positions,levels) => {
  try {

    await page.goto("https://www.median-kliniken.de/de/karriere/stellenangebote-bewerbung/stellenangebote.html?filter%5Bcountr%5D=Grefrath", {
      waitUntil: "load",
      timeout: 0,
    });
    ...
    for (let jobLink of jobLinks) {
      cluster.queue(async ({ page }) => {
        let job = {
          title: "",
          location: "",
          hospital: "MEDIAN Therapiezentrum Haus Grefrath",
          link: "",
          level: "",
          position: "",
          city: "Grefrath",
        ...
        if (positions.map(el => el.toLowerCase()).inlcudes(job.position.toLowerCase())) {
          await save(job);
        }
      });
    }
...
export default MEDIANTherapiezentrum_HausGrefrath;